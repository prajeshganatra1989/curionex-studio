...
from app.services import knowledge_pack_service
...
def _map_error(exc: Exception) -> HTTPException:
    if isinstance(exc, knowledge_pack_service.NotFoundError):
        return HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc))
    if isinstance(exc, knowledge_pack_service.ForbiddenError):
        return HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc))
    if isinstance(exc, knowledge_pack_service.ValidationError):
...
def post_knowledge_pack(
    ...
    current_user: Annotated[User, Depends(require_permission("knowledge_packs.create"))],
) -> KnowledgePackResponse:
...
def patch_knowledge_pack(
    ...
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[User, Depends(require_permission("knowledge_packs.update"))],
) -> KnowledgePackResponse:
    ctx = extract_request_audit_context(request)
    try:
...
def delete_knowledge_pack(
    ...
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[User, Depends(require_permission("knowledge_packs.delete"))],
) -> KnowledgePackResponse:
