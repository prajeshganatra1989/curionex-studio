...
def post_user_role(
    ...
    role_id: UUID,
    db: Annotated[Session, Depends(get_db)],
    _: Annotated[User, Depends(require_permission("roles.assign"))],
) -> MessageResponse:
    try:
        rbac_service.assign_role_to_user(db, user_id=user_id, role_id=role_id)
    except rbac_service.NotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def delete_user_role(
    ...
    try:
        rbac_service.remove_role_from_user(db, user_id=user_id, role_id=role_id)
    except rbac_service.NotFoundError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
